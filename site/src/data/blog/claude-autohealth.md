...
- And then we mixed in Markov modelling - what signals can we detect by training a Markov model on the history?[^0]
...
- [Textual](https://textual.textualize.io/) for TUI dashboard
...
[^0]: This relates to another silly idea: https://bjro.dev/posts/markymarkov
[^1]: When building the TUI, we made sure to check that sub-agents also called the hook - they do, and they inherit the session's id, but has their own agent ids.
